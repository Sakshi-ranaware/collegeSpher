...
export default function HODApplicationDetails() {
  ...
  const [studentConduct, setStudentConduct] = useState('GOOD');
  ...
  const handleApproval = async (status) => {
    if (!hodRemark && status === 'approved') {
      setModalConfig({
        isOpen: true,
        title: 'Remark Required',
        message: 'Please provide a remark before approving the application.',
        type: 'warning'
      });
      return;
    }
    ...
    try {
      ...
      await axios.put(`${API_BASE_URL}/hod/approve/${id}`,
        { status, remark: hodRemark, conduct: studentConduct },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setModalConfig({
        isOpen: true,
        title: 'Success',
        message: 'Action successful! The application status has been updated.',
        type: 'success',
        onConfirm: () => navigate('/hod')
      });
    } catch (err) {
  ...
  const handleDownload = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_BASE_URL}/hod/application/${id}/download`, {
        headers: { Authorization: `Bearer ${token}` },
        responseType: 'blob'
      });
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-purple-700 to-indigo-800 px-8 py-6 relative">
          ...
          <div className="flex flex-col items-center justify-center text-center">
            ...
            <span className={`mt-3 inline-flex items-center px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide border-2 ${application.workflowStage === 'completed' ? 'bg-green-500 border-green-400 text-white' :
        ...
        <div className="px-8 py-8 space-y-8">
                  ...
                  {application.noDuesStatuses.map((dept, idx) => (
                    <tr key={idx} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${dept.status === 'cleared' ? 'bg-green-100 text-green-800' :
          ...
          {(application.workflowStage === 'hod_pending' || application.workflowStage === 'dept_pending') && (
            <section className="bg-gray-50 rounded-xl p-6 border border-gray-200 shadow-inner mt-8">
              <div className="space-y-4 mb-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Conduct and Progress</label>
                  <select
                    value={studentConduct}
                    onChange={(e) => setStudentConduct(e.target.value)}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">HOD Remark</label>
                  <textarea
                    value={hodRemark}
                    onChange={(e) => setHodRemark(e.target.value)}
                    placeholder="Enter your assessment or remarks here..."
                    className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-purple-500 focus:border-purple-500 sm:text-sm"
                    rows="3"
                  />
                </div>
          ...
          {application.workflowStage === 'completed' && (
            <section className="bg-green-50 rounded-xl p-8 border border-green-200 shadow-md mt-8 text-center">
              <h3 className="text-xl font-bold text-green-900 mb-2">Certificate Ready</h3>
              ...
              <button
                ...
                className="inline-flex justify-center items-center px-8 py-4 border border-transparent text-lg font-medium rounded-lg text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 shadow-lg transition-all hover:scale-105"
              >
